...
static INT	armCacheV4Invalidate (LW_CACHE_TYPE  cachetype, PVOID  pvAdrs, size_t  stBytes)
{
    ...
    if (cachetype == INSTRUCTION_CACHE) {
        ...
        if (stBytes >= sizeof(PVOID)) {
            ulEnd = (addr_t)pvAdrs + stBytes - sizeof(PVOID);
            armDCacheInvalidate(pvAdrs, (PVOID)ulEnd, ARMv4_CACHE_LINE_SIZE);
        }
    }
...
** 调用模块: 
*********************************************************************************************************/
static INT	armCacheV4VmmAreaInv (LW_CACHE_TYPE  cachetype, PVOID  pvAdrs, size_t  stBytes)
{
    if (cachetype == INSTRUCTION_CACHE) {
        armICacheInvalidateAll();                                       /*  ICACHE 全部无效             */
                      ...
                      CPCHAR       pcMachineName)
{
    ...
    pcacheop->CACHEOP_pfuncVmmAreaInv = armCacheV4VmmAreaInv;
