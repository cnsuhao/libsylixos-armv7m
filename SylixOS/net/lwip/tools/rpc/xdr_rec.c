...
{
	RECSTREAM *rstrm =
		(RECSTREAM *)sys_malloc(sizeof(RECSTREAM));

	if (rstrm == NULL) {
		(void)fprintf(stderr, "rpc : xdrrec_create: out of memory\n");
		/*
	...
	rstrm->the_buffer = sys_malloc(sendsize + recvsize + BYTES_PER_XDR_UNIT);
	...
	XDR *xdrs)
{
	...
	sys_free(rstrm->the_buffer);
	sys_free((caddr_t)rstrm);
}
