...
/*********************************************************************************************************
  ...
  +-----------+------------+--------------------------------+-------------+
  |  KERN MEM |  DMA ZONE  |          COMMON VMM            | TT or OTHER |
  |           |            |                                |             |
  |  内核内存 |  ATTR_DMA  |          ATTR_NONE             | 页表或其他  |
  +-----------+------------+--------------------------------+-------------+
  
  这里只是举例, 内存的顺序没有要求, 例如, 内核内存可以不必要在最前面, 页表或其他也可以不在最后面.
  
  但是, SylixOS 对 BSP 有以下要求: "内核内存"和具有"ATTR_DMA"属性的DMA内存编址, 不能与虚拟空间有重叠!
  
  "ATTR_NONE"或者"页表或其他"是可以和虚拟空间编址有重叠的, 因为 vmmDmaAlloc 出来的内存地址是物理地址, 而且
  
  他的虚拟地址也是相同的. 也就是说: 内核内存 与 ATTR_DMA 内存必须是平板映射, 而且地址不能与虚拟空间重叠!
  ...
  SylixOS 提供虚拟空间一般比真实的物理空间大很多, 一般在 32 位机器上可以达到 3GB(不能与内核内存, ATTR_DMA
