...
#if __ARCH_DOUBLE_INC_FLOAT_H == 0
#define Bad_float_h                     /*  don't include "float.h"   */
#endif
...
/****************************************************************
  ARM GCC old mixed-endian fixed
****************************************************************/
#if defined(__GNUC__)
#if defined(__arm__)
...
#define IEEE_BIG_ENDIAN     /*  old mixed-endian                */
...
long double
lib_strtold(const char *s, char **se)
{
	return lib_strtod(s, se);
}

double
lib_strtod(CONST char *s00, char **se)
{
	...
	Bigint *b, *b1, *delta, *mlo=NULL, *mhi, *S;
        ...
        if (word0(d)  == 0x8000)
                ...
                *decpt = 9999;
                s =
#ifdef IEEE_Arith
			!word1(d) && !(word0(d) & 0xfffff) ? ndigits < 8 ? (char*)"Inf" : (char*)"Infinity" :
#endif
				(char*)"NaN";
                if (rve)
                        *rve =
