...
UINT inc_lock (	/* Increment file open counter and returns its index (0:int error) */
	FATDIR* dj,	/* Directory object pointing the file to register or increment */
...
{
	FIL *fil = (FIL*)obj;	/* Assuming offset of fs and id in the FIL/FATDIR is identical */
...
)
{
	...
	*bw = 0;	/* Clear write byte counter */
	...
	if (!(fp->flag & FA_WRITE))				/* Check access mode */
		LEAVE_FF(fp->fs, FR_DENIED);
	/* Patch June 22, 2013 */
	if ((DWORD)(fp->fptr + btw) < fp->fsize) btw = 0;	/* File size cannot reach 4GB */
