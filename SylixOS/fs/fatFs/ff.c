...
static
FRESULT dir_find (
	FATDIR* dp			/* Pointer to the directory object linked to the file name */
)
{
	FRESULT res;
	BYTE c, *dir;
...
#ifdef SYLIXOS
        fno->fcdate = LD_WORD(dir+DIR_CrtDate);		/* Date */
		fno->fctime = LD_WORD(dir+DIR_CrtTime);		/* Time */
#endif
...
)
{
    ...
    {
        /**
         *  ADS (armcc) compiler here has a BUG! so use mem_cmp() check.
         */
        static const char fatstr[] = "FAT";
        
        if (!mem_cmp(&fs->win[BS_FilSysType], fatstr, 3))	/* Check FAT signature */
		    return 0;
	    if (!mem_cmp(&fs->win[BS_FilSysType32], fatstr, 3))
		    return 0;
    }
#else
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	sysect = nrsv + fasize + fs->n_rootdir / (SS(fs) / SZ_DIR);	/* RSV+FAT+FATDIR */
...
FRESULT validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
	void* obj		/* Pointer to the object FIL/FATDIR to check validity */
)
{
...
/*-----------------------------------------------------------------------*/
/* Mount/Unmount a Logical Drive for sylixos                             */
/*-----------------------------------------------------------------------*/
...
)
{
	...
	if (fs) {
		fs->fs_type = 0;				/* Clear new fs object */
		fs->drv = LD2PD(vol);           /* save driver number  */
#if _FS_REENTRANT						/* Create sync object for the new volume */
		if (!ff_cre_syncobj((BYTE)vol, &fs->sobj)) return FR_INT_ERR;
#endif
	}
...
)
{
	...
	if (uniq) *uniq = ((uint64_t)dj.sclust << 32) + dj.index;
...
)
{
    ...
    if (uniq) *uniq = ((uint64_t)dp->sclust << 32) + dp->index;
...
#ifdef SYLIXOS
#define N_FATS		LW_CFG_FATFS_NFATS  /* Number of FAT copies (1 or 2) */
#else
#define N_FATS		1
#endif  /* SYLIXOS */
